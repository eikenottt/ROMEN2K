...
def train(train_df):
    ...
    train_df["counter"] = train_df["cleaned_text"].apply(lambda text: collections.Counter())
...
def predict(test_data, trained_df):
    ...
    test_data.drop(["path"], axis=1, inplace=True)
    print(test_data.iterrows())
    ...
    test_data["counter"] = test_data["cleaned_text"].apply(lambda text: collections.Counter())
